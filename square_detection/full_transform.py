...
input_image = "output4100.png"

img = cv2.imread(input_image)
rows,cols,ch = img.shape
gray = cv2.cvtColor(img,cv2.COLOR_BGR2GRAY)

black = np.zeros(shape=(rows,cols))
...
for i in corners:
    x,y = i.ravel()
    corner_dat.append([x,y])
    cv2.circle(black,(x,y),10,[255,255,0],-1)
...
pts1 = np.float32([[line2[2],line2[3]],[line1[2],line1[3]],[line2[0],line2[1]],[line1[0],line1[1]]])
pts2 = np.float32([[0,0],[5000,0],[0,15000],[5000,15000]])

M = cv2.getPerspectiveTransform(pts1,pts2)

dst = cv2.warpPerspective(img,M,(5000,15000))
...
plt.subplot(222),plt.imshow(black),plt.title('Edge Detection')
